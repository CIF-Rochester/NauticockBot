...
class General(commands.Cog):
    ...
    @nextcord.slash_command(
        name="wiki", description="Link to the CIF Wiki", guild_ids=serverIdList
    )
    async def wiki(self, interaction: Interaction):
        await interaction.response.send_message(
            "https://github.com/CIF-Rochester/wiki/wiki"
        )

    @nextcord.slash_command(
        name="website", description="Link to the CIF Website", guild_ids=serverIdList
    )
    ...
    @nextcord.slash_command(
        name="join-cif",
        description="Link to the CIF Application",
    ...
    @nextcord.slash_command(
        name="calendar", description="Link to the CIF Calendar", guild_ids=serverIdList
    )
    async def calendar(self, interaction: Interaction):
        await interaction.response.send_message(
            "https://cif.rochester.edu/events-calendar/"
        )

    @nextcord.slash_command(
        name="lab",
        description="Link to the CIF Lab Access Form",
        guild_ids=serverIdList,
    )
    ...
    @nextcord.slash_command(
        name="github", description="Link to the CIF Github", guild_ids=serverIdList
    )
    ...
    @nextcord.slash_command(
        name="source-code",
        description="Link to The Nauticock Bot Source Code",
        guild_ids=serverIdList,
    )
    async def source_code(self, interaction: Interaction):
        await interaction.response.send_message(
            "https://github.com/CIF-Rochester/NauticockBot"
        )

    @nextcord.slash_command(
        name="help", description="Help for using the bot", guild_ids=serverIdList
    )
